  ...
  this.update = () => new Promise((resolveUpdate, rejectUpdate) => {
    ...
    const promiseGetPeers = new Promise((resolve) => {
      daemon.getPeers()
		...
		peers = JSON.parse(node)
	    if (peers.hasOwnProperty('white_list')) {
                this.peers = node.white_list.map(peer => {
        ...
        .catch((err) => {
          ...
          this.error = {status:true, msg:`Module:defaultNode.js, Method:promiseGetPeers ${err} `};
          resolve(this);
        });
    });
    ...
    Promise.all([
      ...
      if (this.error.status) {
        rejectUpdate(new Error(this.error.msg));
      } else {
        resolveUpdate(this);
      }
    });
